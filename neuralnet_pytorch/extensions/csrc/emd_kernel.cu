...
__global__ void
approxmatch(int b, int n, int m, const scalar_t* __restrict__ xyz1,
    ...
    for (int j = 7; j >= -2; j--) {
      ...
      for (int l0 = 0; l0 < m; l0 += blockDim.x) {
        ...
        if (l < m) {
          ...
          remainR[l] = fmaxf(scalar_t(0.0), remainR[l] - sumr);
        }
      }
      __syncthreads();
      for (int k0 = 0; k0 < n; k0 += blockDim.x) {
        ...
        if (k < n)
          remainL[k] = fmaxf(scalar_t(0.0), remainL[k] - suml);
      }
      __syncthreads();
    }
  }
...
*/
at::Tensor
approx_match_cuda_forward(const at::Tensor xyz1, const at::Tensor xyz2)
{
  ...
  AT_DISPATCH_FLOATING_TYPES_AND(
    at::ScalarType::Half, xyz1.scalar_type(), "approx_match_forward_cuda",
    ([&] {
...
template <typename scalar_t>
__global__ void
matchcost(int b, int n, int m, const scalar_t* __restrict__ xyz1,
          const scalar_t* __restrict__ xyz2, const scalar_t* __restrict__ match,
...
*/
at::Tensor
match_cost_cuda_forward(const at::Tensor xyz1, const at::Tensor xyz2,
                        const at::Tensor match)
{
  ...
  AT_DISPATCH_FLOATING_TYPES_AND(
    at::ScalarType::Half, xyz1.scalar_type(), "match_cost_forward_cuda", ([&] {
...
template <typename scalar_t>
__global__ void
matchcostgrad2(int b, int n, int m, const scalar_t* __restrict__ grad_cost,
               const scalar_t* __restrict__ xyz1,
               const scalar_t* __restrict__ xyz2,
               const scalar_t* __restrict__ match, scalar_t* __restrict__ grad2)
{
...
template <typename scalar_t>
__global__ void
matchcostgrad1(int b, int n, int m, const scalar_t* __restrict__ grad_cost,
               const scalar_t* __restrict__ xyz1,
               const scalar_t* __restrict__ xyz2,
               const scalar_t* __restrict__ match, scalar_t* __restrict__ grad1)
{
  for (int i = blockIdx.x; i < b; i += gridDim.x) {
...
match_cost_cuda_backward(const at::Tensor grad_cost, const at::Tensor xyz1,
                         const at::Tensor xyz2, const at::Tensor match)
{
  ...
  auto grad2 = at::zeros({ b, m, 3 }, xyz2.type());

  AT_DISPATCH_FLOATING_TYPES_AND(
    at::ScalarType::Half, xyz1.scalar_type(), "match_cost_backward_cuda", ([&] {
      matchcostgrad1<scalar_t><<<32, 512>>>(
        b, n, m, grad_cost.data<scalar_t>(), xyz1.data<scalar_t>(),
        xyz2.data<scalar_t>(), match.data<scalar_t>(), grad1.data<scalar_t>());
